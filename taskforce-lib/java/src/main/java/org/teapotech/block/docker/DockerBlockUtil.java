 ...
 *
 */
public class DockerBlockUtil {

	public static String LABEL_TASK_NAME = "taskforce.task.name";

	final DockerClient dockerClient;

	public DockerBlockUtil(DockerClient dockerClient) {
	...
	public void runDockerTask(DockerBlockDescriptor taskDescriptor) throws InvalidBlockException {
		String tag = taskDescriptor.getName() + ":" + taskDescriptor.getVersion();
		try {
			List<Image> images = dockerClient.listImages(DockerClient.ListImagesParam.byName(tag));
			if (images == null || images.isEmpty()) {
				throw new InvalidBlockException("Docker task image does not exist. Name: " + tag);
			}
			Image img = images.get(0);
			// dockerClient.createContainer(arg0)
		} catch (DockerException | InterruptedException e) {
			throw new InvalidBlockException(e.getMessage(), e);
		}
	}
