...
public class TaskforceEntityQuerySpecs {
	...
	public static Specification<SimpleTaskforceEntity> querySimpleTaskforceEntity(String id, String name,
			SimpleTaskforceGroup group) {
		return new Specification<SimpleTaskforceEntity>() {
			private static final long serialVersionUID = 418739678937855928L;

			@Override
			public Predicate toPredicate(Root<SimpleTaskforceEntity> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				List<Predicate> predicates = new ArrayList<Predicate>();
				...
				if (group != null) {
					predicates.add(cb.equal(root.<SimpleTaskforceGroup>get("group"), group));
				}
